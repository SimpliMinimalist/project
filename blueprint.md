...
- **Smarter Draft Load Confirmation**: The confirmation dialog for loading a draft is now only shown if the user has made changes to the current form. This prevents unnecessary interruptions when the form is empty.
- **Save or Discard Workflow**: When loading a new draft while there are unsaved changes in the current form, the user is now prompted to either save the current work as a new draft, discard the changes, or cancel the action. This provides a more intuitive and user-friendly workflow.
